...
async function handleSuttaSubmit(e) {
    ...
    if (isSavingSutta) {
        showToast('දත්ත සුරැකීම දැනට ක්‍රියාත්මකයි...', 'info');
        return;
    }
    ...
    const status = getInputValue('sutta_status') || 'published';

    // --- Passages එකතු කිරීම (Full Text පමණයි) ---
    ...
    for (let i = 0; i < maxLen; i++) {
        ...
        if (pali || sinhala) passages.push({ pali, sinhala });
    }

    // --- Glossary එකතු කිරීම (Bulk පමණයි) ---
        ...
        lines.forEach(line => {
            ...
            if (trimmed) {
                ...
                if (parts.length >= 2) {
                    ...
                    if (word || meaning) glossary.push({ word, meaning });
                }
            }
        });
    }
    ...
    try {
        let existingData = [];
        try {
            const res = await fetch('data/suttas.json?t=' + new Date().getTime());
            if (res.ok) existingData = await res.json();
            if (!Array.isArray(existingData)) existingData = [];
        } catch (_) { /* ගොනුව නොමැති නම් හිස් අරාවක් */ }

        // සංස්කරණය හෝ එකතු කිරීම
        const editIndex = existingData.findIndex(s => s.id === (editId || suttaId));
        if (editIndex !== -1) {
            existingData[editIndex] = newSutta;
        } else {
            // ID ගැටුමක් ඇත්නම් අනතුරු අඟවන්න
            if (existingData.some(s => s.id === newSutta.id)) {
                throw new Error('මෙම ID එක සහිත සූත්‍රයක් දැනටමත් පවතී.');
            }
            existingData.push(newSutta);
        }

        // JSON ගොනුව ඩවුන්ලෝඩ් කරන්න
        downloadJSON(existingData);

        showToast('සූත්‍රය සාර්ථකව සුරක්ෂිත කරන ලදී! suttas.json ඩවුන්ලෝඩ් විය. එය data/ ෆෝල්ඩරයට උඩුගත කරන්න.', 'success');
        resetSuttaForm();
        await loadSuttasTable();

    } catch (err) {
        console.error('Save error:', err);
        showToast('දත්ත සුරැකීමේදී දෝෂයක්: ' + err.message, 'error', 7000);
    } finally {
        isSavingSutta = false;
...
// --- JSON ඩවුන්ලෝඩ් කිරීමේ උපකාරක ශ්‍රිතය ---
...
async function loadSuttasTable() {
    ...
    try {
        const response = await fetch('data/suttas.json?t=' + new Date().getTime());
        if (!response.ok) throw new Error('ගොනුව හමු නොවීය');
        ...
        tbody.innerHTML = '<tr><td colspan="6" class="loading-msg" style="color:#dc2626;">suttas.json හමු නොවීය හෝ දත්ත වැරදියි. කරුණාකර ගොනුව පරීක්ෂා කරන්න.</td></tr>';
    }
}
...
async function editSutta(id) {
    try {
        const response = await fetch('data/suttas.json?t=' + new Date().getTime());
        if (!response.ok) throw new Error('ගොනුව හමු නොවීය');
        const allSuttas = await response.json();
        const sutta = allSuttas.find(s => s.id === id);
        if (!sutta) throw new Error('සූත්‍රය හමු නොවීය');

        // --- පෝරමය පිරවීම (කලින් තිබූ ආකාරයටම) ---
        document.getElementById('edit_sutta_id').value = sutta.id;
        document.getElementById('sutta_id_input').value = sutta.id;
        document.getElementById('sutta_id_input').disabled = true;
        document.getElementById('title').value = sutta.title || '';
        document.getElementById('subtitle').value = sutta.subtitle || '';
        document.getElementById('order_no').value = sutta.order_no ?? 1;
        document.getElementById('pitaka').value = sutta.pitaka || '';
        document.getElementById('nikaya').value = sutta.nikaya || '';
        document.getElementById('vagga').value = sutta.vagga || '';
        document.getElementById('speaker').value = sutta.speaker || '';
        document.getElementById('category').value = sutta.category || '';
        document.getElementById('sutta_status').value = sutta.status || 'published';
        document.getElementById('formTitleText').innerText = 'සූත්‍රය සංස්කරණය කිරීම (' + (sutta.title || '') + ')';

        // Passages (Full Text)
        let passages = sutta.passages || [];
        if (typeof passages === 'string') {
            try { passages = JSON.parse(passages); } catch (e) { passages = []; }
        }
        document.getElementById('full_pali_text').value = passages.map(p => p.pali || '').join('\n\n');
        document.getElementById('full_sinhala_text').value = passages.map(p => p.sinhala || '').join('\n\n');
        document.getElementById('input_method').value = 'full_text';
        toggleInputMethod();

        // Glossary (Bulk)
        let glossary = sutta.glossary || [];
        if (typeof glossary === 'string') {
            try { glossary = JSON.parse(glossary); } catch (e) { glossary = []; }
        }
        document.getElementById('full_glossary_text').value = glossary.map(g => (g.word || '') + ' – ' + (g.meaning || '')).join('\n');
        document.getElementById('glossary_input_method').value = 'bulk_paste';
        toggleGlossaryInputMethod();

        // Clear dynamic rows (just in case)
        document.getElementById('passages_dynamic_rows').innerHTML = '';
        document.getElementById('glossary_dynamic_rows').innerHTML = '';
...
async function deleteSutta(id) {
    if (!confirm(`'${id}' අංකනය සහිත සූත්‍රය මකා දැමීමට අවශ්‍යද? (යාවත්කාලීන JSON ගොනුව ඩවුන්ලෝඩ් වේ)`)) {
        return;
    }
    ...
    try {
        const response = await fetch('data/suttas.json?t=' + new Date().getTime());
        if (!response.ok) throw new Error('ගොනුව හමු නොවීය');
        let allSuttas = await response.json();

        // ID එකට ගැලපෙන සූත්‍රය ඉවත් කරන්න
        allSuttas = allSuttas.filter(s => s.id !== id);

        // JSON ගොනුව ඩවුන්ලෝඩ් කරන්න
        downloadJSON(allSuttas);

        showToast(`'${id}' මකා දමන ලදී. suttas.json ඩවුන්ලෝඩ් විය. එය data/ ෆෝල්ඩරයට උඩුගත කරන්න.`, 'success');
        await loadSuttasTable();

    } catch (err) {
        showToast('මකා දැමීමේ දෝෂයකි: ' + err.message, 'error');
    }
}
